...
const Audits: FC<{}> = (): ReactElement => {
    ...
    useEffect(() => {
        ...
        getJsonApi<AuditsCollection>(context.settings.auditEndpoint + "?page[limit]=" + pageSize + "&page[offset]=" + (page * pageSize), context.settings, context.partition)
            .then(data => {
                setAudits(data);
    ...
    const refresh = () => {
        setAudits(null);
        setError(null);
        getJsonApi<AuditsCollection>(context.settings.auditEndpoint + "?page[offset]=" + (page * pageSize) + "&page[limit]=" + pageSize, context.settings, context.partition)
            .then(data => {
                setAudits(data);
                setRows(data.links?.first?.meta?.total ?? FALLBACK_ROW_COUNT);
            })
