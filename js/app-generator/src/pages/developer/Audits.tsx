...
const Audits: FC<{}> = (): ReactElement => {
    ...
    const columns = [
        ...
        {field: 'object', headerName: 'Object', width: 240},
    ...
    useEffect(() => {
        ...
        getJsonApi<AuditsCollection>(context.settings.auditEndpoint + "?filter=subject==GithubRepo&page[limit]=" + pageSize + "&page[offset]=" + (page * pageSize), context.settings, context.partition)
            .then(data => {
                setAudits(data);
    ...
    }, [setAudits, pageSize, page, context.settings.auditEndpoint, context.partition, context.settings]);

    const refresh = () => {
        setAudits(null);
        setError(null);
        getJsonApi<AuditsCollection>(context.settings.auditEndpoint + "?filter=subject==GithubRepo&page[offset]=" + (page * pageSize) + "&page[limit]=" + pageSize, context.settings, context.partition)
            .then(data => {
                setAudits(data);
                setRows(data.links?.first?.meta?.total ?? FALLBACK_ROW_COUNT);
            })
