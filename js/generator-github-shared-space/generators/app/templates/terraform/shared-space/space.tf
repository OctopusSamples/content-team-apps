...
resource "octopusdeploy_space" "app_builder_space" {
  ...
  name                        = "${substr("${var.octopus_space} ${lower(var.github_repo_owner)}", 0, 18)} ${data.octopusdeploy_spaces.app_builder_space.count == 0 ? substr("${var.octopus_space} ${lower(var.github_repo_owner)}", 19, 2) : data.octopusdeploy_spaces.app_builder_space.count + 1}"
  is_default                  = false
