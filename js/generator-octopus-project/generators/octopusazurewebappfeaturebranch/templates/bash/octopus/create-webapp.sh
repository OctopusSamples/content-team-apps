NOW=$(date +%s)
CREATED=$${NOW}
FEATUREBRANCH="#{if Octopus.Action[Deploy WebApp].Package[].PackageVersion | VersionPreReleasePrefix}#{Octopus.Action[Deploy WebApp].Package[].PackageVersion | VersionPreReleasePrefix}#{/if}#{unless Octopus.Action[Deploy WebApp].Package[].PackageVersion | VersionPreReleasePrefix}master#{/unless}"
