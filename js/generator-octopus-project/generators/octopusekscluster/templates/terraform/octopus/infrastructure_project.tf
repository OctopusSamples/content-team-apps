    ...
    action {
      ...
      properties     = {
        ...
        "Octopus.Action.Script.ScriptBody": <<-EOT
          ...
          # So we just use "-i".
          INDEX=$(aws eks list-clusters | jq ".clusters | index(\"${lower(var.github_owner)}-${lower(var.github_repo)}-$${FIXED_ENVIRONMENT}\")")
          ...
          metadata:
            name: ${lower(var.github_owner)}-${lower(var.github_repo)}-$${FIXED_ENVIRONMENT}
      ...
      properties     = {
        ...
        "Octopus.Action.Script.ScriptBody" : <<-EOT
          ...
          eksctl utils associate-iam-oidc-provider \
              --region=${var.aws_region} \
              --cluster=${lower(var.github_owner)}-${lower(var.github_repo)}-$${FIXED_ENVIRONMENT} \
              --approve
          ...
          aws eks update-kubeconfig --name ${lower(var.github_owner)}-${lower(var.github_repo)}-$${FIXED_ENVIRONMENT} --kubeconfig /build/kubeconfig
          ...
          sed -i.bak -e "s|your-cluster-name|${lower(var.github_owner)}-${lower(var.github_repo)}-$${FIXED_ENVIRONMENT}|" ./v2_4_1_full.yaml
    ...
    action {
      ...
      properties     = {
        ...
        "Octopus.Action.Script.ScriptBody": <<-EOT
          ...
          aws eks describe-cluster --name ${lower(var.github_owner)}-${lower(var.github_repo)}-$${FIXED_ENVIRONMENT} > clusterdetails.json
          ...
          echo "##octopus[create-kubernetestarget \
            ...
            clusterName=\"$(encode_servicemessagevalue "${lower(var.github_owner)}-${lower(var.github_repo)}-$${FIXED_ENVIRONMENT}")\" \
            clusterUrl=\"$(encode_servicemessagevalue "$(cat clusterdetails.json | jq -r '.cluster.endpoint')")\" \
            octopusAccountIdOrName=\"$(encode_servicemessagevalue "#{AWS.Account}")\" \
