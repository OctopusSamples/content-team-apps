resource "github_actions_secret" "development_azure_account" {
  ...
  plaintext_value = octopusdeploy_azure_service_principal.development_azure_account.id
}
...
resource "github_actions_secret" "production_aws_access_key" {
  ...
  plaintext_value = octopusdeploy_azure_service_principal.production_azure_account.id
}
