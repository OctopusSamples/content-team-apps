                ...
                .build(Build.builder()
                    ...
                    .steps(new ImmutableList.Builder<Step>()
                        ...
                        .add(RunStep.builder()
                            ...
                            .shell("bash")
                            .run("file=\"${{ steps.get_artifact.outputs.artifact }}\"; filename=\"${file##*/}\"; filenoextension=\"${filename%.*}\"; extension=\"${filename##*.}\" octofile=\"${filenoextension}.0.1.${{ github.run_number }}.${extension}\"; cp ${file} ${octofile}; echo \"::set-output name=artifact::${octofile}\"")
                            .build())
                        .add(RunStep.builder()
                            .name("Push to Octopus")
                            .shell("bash")
                            .run("octo push --package \"${{ steps.get_octopus_artifact.outputs.artifact }}\" --server ${{ secrets.OCTOPUS_SERVER_URL }} --apiKey ${{ secrets.OCTOPUS_API_TOKEN }}")
                            .build())
                        .add(RunStep.builder()
